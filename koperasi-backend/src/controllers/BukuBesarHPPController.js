// src/controllers/BukuBesarHPPController.js
...
class BukuBesarHPPController {
  // ─── Build data buku besar HPP ─────────────────────────────
  async buildData(akun, tanggalMulai, tanggalSelesai) {
  ...
  // ─── Ambil akun HPP pertama ──────────────────────────────
  async getAkunHPP() {
    const akun = await Akun.findOne({
      where: { nama_akun: { [Op.like]: '%hpp%' } },
    });
    if (!akun) {
      throw new Error('Akun HPP tidak ditemukan');
    }
    return akun;
  }

  // ─── ENDPOINT: GET /api/bendahara/buku-besar-hpp ──────────
  async index(req, res) {
    try {
      const { tanggal_mulai, tanggal_selesai } = req.query;
      const now = new Date();
      const defaultMulai = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0, 10);
      const defaultSelesai = now.toISOString().slice(0, 10);

      const mulai = tanggal_mulai || defaultMulai;
      const selesai = tanggal_selesai || defaultSelesai;

      const akun = await this.getAkunHPP();
      const result = await this.buildData(akun, mulai, selesai);
      ...
      console.error('Error Buku Besar HPP:', error);
      res.status(500).json({
        message: error.message || 'Gagal mengambil data buku besar HPP',
        error: error.message,
      });
    }
  }

  // ─── ENDPOINT: GET /api/bendahara/buku-besar-hpp/export ──
  async export(req, res) {
    try {
      ...
      if (exportType === 'excel') {
        await this.exportExcel(res, akun, result, mulai, selesai);
      } else if (exportType === 'pdf') {
        await this.exportPdf(res, akun, result, mulai, selesai);
      } else {
        res.status(400).json({ message: 'Format export tidak didukung' });
      }
    } catch (error) {
      console.error('Error export Buku Besar HPP:', error);
      res.status(500).json({
        message: error.message || 'Gagal mengekspor buku besar HPP',
        error: error.message,
      });
    }
  }
  ...
  // ─── Export Excel ──────────────────────────────────────────
  async exportExcel(res, akun, result, mulai, selesai) {
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('Buku Besar HPP');

    // Header
    sheet.mergeCells('A1:E1');
    sheet.getCell('A1').value = `BUKU BESAR HPP - ${akun.kode_akun} ${akun.nama_akun}`;
    ...
    res.setHeader('Content-Disposition', `attachment; filename=buku-besar-hpp-${akun.kode_akun}-${new Date().toISOString().slice(0, 10)}.xlsx`);
    await workbook.xlsx.write(res);
    res.end();
  }

  // ─── Export PDF ─────────────────────────────────────────────
  async exportPdf(res, akun, result, mulai, selesai) {
    const doc = new PDFDocument({ margin: 40, size: 'A4', layout: 'landscape' });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=buku-besar-hpp-${akun.kode_akun}-${new Date().toISOString().slice(0, 10)}.pdf`);
    doc.pipe(res);

    // Header
    doc.fontSize(14).font('Helvetica-Bold').text(
      `BUKU BESAR HPP - ${akun.kode_akun} ${akun.nama_akun}`,
      { align: 'center' }
    );
    doc.fontSize(10).font('Helvetica').text(
      `Periode: ${formatTanggal(mulai)} - ${formatTanggal(selesai)}`,
      { align: 'center' }
    );
    doc.moveDown(0.5);
...
module.exports = new BukuBesarHPPController();