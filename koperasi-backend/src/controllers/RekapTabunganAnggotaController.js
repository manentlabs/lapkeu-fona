// src/controllers/RekapTabunganAnggotaController.js
// 
// Controller untuk KARTU TABUNGAN ANGGOTA.
// Menggunakan tabel dan model khusus untuk Tabungan (bukan Simpanan).
// Asumsi: ada model JenisTabungan, TabunganAwal, dan kolom jenis_tabungan_id di transaksi.

const { Anggota, TabunganAwal, Transaksi, JenisTabungan, sequelize } = require('../models');
...
// ─── Controller ──────────────────────────────────────────
class RekapTabunganAnggotaController {
  // ─── Ambil daftar jenis tabungan ──
  async getJenisTabungan() {
    return await JenisTabungan.findAll({
      where: { is_active: true }, // bisa tambahkan filter jika perlu
      order: [['urutan', 'ASC'], ['id', 'ASC']],
      attributes: ['id', 'kode', 'nama', 'kolom_key'],
      raw: true,
    });
  }
  ...
  // ─── Hitung saldo awal dari TabunganAwal dan transaksi sebelum tanggal mulai ──
  async getSaldoAwal(anggotaId, jenisTabunganIds, tanggalMulai) {
    if (jenisTabunganIds.length === 0) return 0;

    // Saldo awal dari tabel tabungan_awal (asumsi model TabunganAwal)
    const [saldoAwalRow] = await sequelize.query(
      `
        SELECT COALESCE(SUM(ta.jumlah), 0) AS total
        FROM tabungan_awal ta
        WHERE ta.anggota_id = :anggotaId
          AND ta.jenis_tabungan_id IN (:jenisTabunganIds)
      `,
      {
        replacements: { anggotaId, jenisTabunganIds },
    ...
    // Jika ada tanggal mulai, tambahkan transaksi sebelum tanggal tersebut
    if (tanggalMulai) {
      const [transaksiSebelum] = await sequelize.query(
        `
          ...
          WHERE t.anggota_id = :anggotaId
            AND t.jenis_tabungan_id IN (:jenisTabunganIds)
            AND t.tanggal < :tanggalMulai
        `,
        {
          replacements: { anggotaId, jenisTabunganIds, tanggalMulai },
          type: QueryTypes.SELECT,
        }
      );
  ...
  // ─── Ambil daftar transaksi berdasarkan jenis_tabungan_id ──
  async getTransaksiAnggota(anggotaId, jenisTabunganIds, tanggalMulai, tanggalSelesai, search) {
    if (jenisTabunganIds.length === 0) return [];

    let where = `
      WHERE t.anggota_id = :anggotaId
        AND t.jenis_tabungan_id IN (:jenisTabunganIds)
    `;
    const replacements = { anggotaId, jenisTabunganIds };
    ...
    const query = `
      SELECT
        ...
        jt.kolom_key       AS jenis_key
      FROM transaksi t
      LEFT JOIN jenis_tabungan jt ON jt.id = t.jenis_tabungan_id
      ${where}
  ...
  // ─── Susun kartu tabungan ──
  async buildKartuTabungan(anggota, jenisList, filters) {
    const { tanggal_mulai, tanggal_selesai, search, jenis_tabungan_id } = filters;

    let filteredJenis = jenisList;
    if (jenis_tabungan_id) {
      filteredJenis = jenisList.filter((j) => Number(j.id) === Number(jenis_tabungan_id));
    }
    ...
    const transaksi = rawTransaksi.map((t) => {
      const jenis = jenisById[t.jenis_tabungan_id];
      const jumlahEfektif = parseFloat(t.jumlah) || 0;

      saldoBerjalan += jumlahEfektif;

      if (jenis && berjalanPerJenis[jenis.id] !== undefined) {
        berjalanPerJenis[jenis.id] += jumlahEfektif;
      }
  ...
  // ─── ENDPOINT: GET /api/rekap-tabungan-anggota ──
  async index(req, res) {
    try {
      const {
        nama_anggota,
      ...
      const jenisTabunganOut = jenisList.map((j) => ({
        id: j.id,
        nama: j.nama,
        kolom_key: j.kolom_key,
      }));
      ...
      if (!nama_anggota) {
        ...
        return res.status(200).json({
          success: true,
          jenisTabungan: jenisTabunganOut,
          namaAnggota,
          transaksi: [],
          detailPerJenis: [],
          grandTotal: {},
      ...
      const { saldoAwal, transaksi, detailPerJenis, grandTotal } = await this.buildKartuTabungan(
        anggota,
        jenisList,
        { tanggal_mulai, tanggal_selesai, search, jenis_tabungan_id }
      );
      ...
      console.error('❌ Error RekapTabunganAnggota:', error);
      return res.status(500).json({
        success: false,
        message: 'Gagal mengambil data rekap tabungan anggota',
        error: error.message,
      });
    }
  }
  ...
  async export(req, res) {
    try {
      ...
      if (transaksi.length === 0 && saldoAwal === 0) {
        return res.status(404).json({
          success: false,
          message: 'Tidak ada transaksi tabungan untuk anggota tersebut pada periode ini.',
        });
      }

      const exportData = { anggota, saldoAwal, transaksi, detailPerJenis, grandTotal, title: 'Tabungan' };
  ...
  // ─── Export Excel (sama dengan sebelumnya, hanya title berubah) ──
...
module.exports = new RekapTabunganAnggotaController();