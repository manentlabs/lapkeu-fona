// controllers/PiutangController.js
...
class PiutangController {
  ...
  // ----------------------------------------------------------
  // BUILD PIVOT QUERY (Piutang = total Debit - total Kredit)
  // ----------------------------------------------------------
  async buildPivotQuery(
    ...
    // Untuk piutang, tidak ada saldo awal (bisa ditambahkan jika ada tabel piutang_awal)
    // Transaksi Debit (piutang bertambah - saat pinjaman)
    ...
    transaksiDebetSub += `
      GROUP BY t.anggota_id, t.akun_debet_id, t.jenis_piutang_id
    `;

    // Transaksi Kredit (piutang berkurang - saat pembayaran)
    ...
    transaksiKreditSub += `
      GROUP BY t.anggota_id, t.akun_kredit_id, t.jenis_piutang_id
    `;

    // Pemetaan Jenis Piutang -> Akun
    const jenisAkunSub = `
      SELECT
        jp.id AS jenis_id,
        jp.kolom_key,
      ...
      FROM jenis_piutang jp
      WHERE jp.is_active = 1
        AND jp.id IN (${jenisIds.join(",")})
        AND jp.akun_id IS NOT NULL
    `;

    // Query Utama: saldo piutang = total debet - total kredit
    const query = `
      WITH
      ...
      combined AS (
        ...
        LEFT JOIN transaksi_debet td
          ...
          AND (
            td.jenis_piutang_id = ja.jenis_id
            OR (td.jenis_piutang_id IS NULL AND ja.akun_share_count = 1)
          )
        LEFT JOIN transaksi_kredit tk
          ON tk.anggota_id = a.id
          AND tk.akun_id = ja.akun_id
          AND (
            tk.jenis_piutang_id = ja.jenis_id
            OR (tk.jenis_piutang_id IS NULL AND ja.akun_share_count = 1)
          )
      ...
      SELECT
        ...
        nama,
        ${pivotSelects},
        SUM(total_akhir) AS total_piutang
      FROM combined
    ...
    try {
      ...
      if (jenisList.length === 0) {
        return res.json({
          ...
          summary: { saldo: {}, totalPiutang: 0, jumlahAnggota: 0 },
          jenisPiutang: [],
          ...
          filterActive: {
            ...
            search: search || "",
            jenis_piutang_id: jenis_piutang_id || "",
          },
          message: "Belum ada jenis piutang yang dikonfigurasi.",
        });
      }
      ...
      if (!result) {
        return res.json({
          ...
          pagination: { page: 1, per_page: Number(per_page) || 10, total: 0, total_pages: 0 },
          summary: { saldo: {}, totalPiutang: 0, jumlahAnggota: 0 },
          jenisPiutang: jenisList,
          filters: { daftarAnggota: [], daftarNoAnggota: [] },
          message: "Jenis piutang belum memiliki akun.",
        });
      }
      ...
      const filteredJenis = jenis_piutang_id
        ? jenisList.filter((j) => Number(j.id) === Number(jenis_piutang_id))
        : jenisList;
      ...
      const totalPiutang = allData.reduce((sum, item) => sum + (parseFloat(item.total_piutang) || 0), 0);
      ...
      return res.json({
        ...
        jenisPiutang: jenisList.map((jenis) => ({
          id: jenis.id,
          kode: jenis.kode,
          nama: jenis.nama,
          kolom_key: jenis.kolom_key,
      ...
      console.error("ERROR PIUTANG:", error);
      return res.status(500).json({ message: "Gagal mengambil data piutang", error: error.message });
    }
  }
  ...
  async export(req, res) {
    try {
      ...
      const jenisList = await this.getJenisPiutang();
      if (jenisList.length === 0) {
        return res.status(422).json({ message: "Belum ada jenis piutang yang dikonfigurasi." });
      }
      ...
      if (!result) {
        return res.status(422).json({ message: "Jenis piutang belum memiliki akun." });
      }
      ...
      const totalAkhir = data.reduce((sum, item) => sum + (parseFloat(item.total_piutang) || 0), 0);

      if (exportType === "excel") {
        return this.exportExcel(res, data, kolomKeys, labels, totals, totalAkhir, "Piutang");
      }
      if (exportType === "pdf") {
        return this.exportPdf(res, data, kolomKeys, labels, totals, totalAkhir, "Piutang");
      }
      return res.status(400).json({ message: "Format export tidak didukung." });
    } catch (error) {
      console.error("ERROR EXPORT PIUTANG:", error);
      return res.status(500).json({ message: "Gagal mengekspor data piutang", error: error.message });
    }
  }
  ...
  // EXPORT EXCEL
  // ----------------------------------------------------------
  async exportExcel(res, data, kolomKeys, labels, totals, totalAkhir, title = "Piutang") {
    ...
    data.forEach((item) => {
      ...
      row.push(parseFloat(item.total_piutang) || 0);
      sheet.addRow(row);
    });
  ...
  // EXPORT PDF
  // ----------------------------------------------------------
  async exportPdf(res, data, kolomKeys, labels, totals, totalAkhir, title = "Piutang") {
    ...
    data.forEach((item) => {
      ...
      row.push(formatRupiah(item.total_piutang || 0));
      drawRow(row);
    });
