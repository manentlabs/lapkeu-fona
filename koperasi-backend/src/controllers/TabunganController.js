// controllers/TabunganController.js
...
class TabunganController {
  ...
  async getJenisTabungan() {
    return await JenisTabungan.findAll({
  ...
  // ----------------------------------------------------------
  //
  // ✅ Penyesuaian: transaksi sekarang menyimpan `jenis_tabungan_id` secara
  // eksplisit (diisi dari dropdown "Jenis Tabungan" di form transaksi).
  // Sebelumnya, pemetaan transaksi -> jenis tabungan HANYA mengandalkan
  // akun_id, sehingga jika 2+ jenis tabungan memakai akun GL yang sama,
  // nominalnya tidak bisa dibedakan (akan tercampur/salah kolom).
  //
  // Sekarang pencocokan mengutamakan t.jenis_tabungan_id = jenis_id.
  // Untuk transaksi lama yang belum punya jenis_tabungan_id (NULL),
  // dipakai fallback ke akun_id — TAPI hanya jika akun tersebut memang
  // unik dipakai oleh satu jenis tabungan saja (akun_share_count = 1),
  // supaya tidak salah atribusi ke jenis yang salah.
  //
  async buildPivotQuery(
    ...
    // ---- Saldo Awal Tabungan ----
    // Sudah menyimpan jenis_tabungan_id langsung, tidak ambigu.
    const saldoAwalSub = `
      SELECT
        ta.anggota_id,
        ta.jenis_tabungan_id,
        SUM(ta.jumlah) AS total_saldo_awal
      FROM tabungan_awal ta
      WHERE ta.jenis_tabungan_id IN (${jenisIds.join(",")})
      GROUP BY ta.anggota_id, ta.jenis_tabungan_id
    `;

    // ---- Transaksi Kredit (tabungan bertambah) ----
    // Sertakan t.jenis_tabungan_id agar pencocokan ke jenis tabungan
    // tidak hanya mengandalkan akun_id.
    ...
    transaksiKreditSub += `
      GROUP BY t.anggota_id, t.akun_kredit_id, t.jenis_tabungan_id
    `;

    // ---- Transaksi Debit (tabungan berkurang) ----
    ...
    transaksiDebetSub += `
      GROUP BY t.anggota_id, t.akun_debet_id, t.jenis_tabungan_id
    `;

    // ---- Pemetaan Jenis Tabungan -> Akun ----
    // akun_share_count menandai berapa banyak jenis tabungan yang memakai
    // akun GL yang sama. Dipakai untuk menentukan apakah fallback akun_id
    // (untuk transaksi lama tanpa jenis_tabungan_id) aman dipakai.
    const jenisAkunSub = `
      SELECT
        jt.id AS jenis_id,
        jt.kolom_key,
        jt.akun_id,
        COUNT(*) OVER (PARTITION BY jt.akun_id) AS akun_share_count
      FROM jenis_tabungan jt
      WHERE jt.is_active = 1
        AND jt.id IN (${jenisIds.join(",")})
        AND jt.akun_id IS NOT NULL
    `;
    ...
    const query = `
      ...
      combined AS (
        ...
        LEFT JOIN saldo_awal sa
          ON sa.anggota_id = a.id AND sa.jenis_tabungan_id = ja.jenis_id
        LEFT JOIN transaksi_kredit tk
          ON tk.anggota_id = a.id
          AND tk.akun_id = ja.akun_id
          AND (
            tk.jenis_tabungan_id = ja.jenis_id
            OR (tk.jenis_tabungan_id IS NULL AND ja.akun_share_count = 1)
          )
        LEFT JOIN transaksi_debet td
          ON td.anggota_id = a.id
          AND td.akun_id = ja.akun_id
          AND (
            td.jenis_tabungan_id = ja.jenis_id
            OR (td.jenis_tabungan_id IS NULL AND ja.akun_share_count = 1)
          )
      ...
      SELECT
        ...
        nama,
        ${pivotSelects},
        SUM(total_akhir) AS total_tabungan
      FROM combined
  ...
  async index(req, res) {
    try {
      ...
      const jenisList = await this.getJenisTabungan();
      if (jenisList.length === 0) {
        return res.json({
          data: [],
          pagination: { page: 1, per_page: Number(per_page) || 10, total: 0, total_pages: 0 },
          summary: { saldo: {}, totalTabungan: 0, jumlahAnggota: 0 },
          jenisTabungan: [],
          ...
          filterActive: {
            ...
            search: search || "",
            jenis_tabungan_id: jenis_tabungan_id || "",
          },
          message: "Belum ada jenis tabungan yang dikonfigurasi.",
        });
      }
      ...
      if (!result) {
        return res.json({
          ...
          pagination: { page: 1, per_page: Number(per_page) || 10, total: 0, total_pages: 0 },
          summary: { saldo: {}, totalTabungan: 0, jumlahAnggota: 0 },
          jenisTabungan: jenisList,
          filters: { daftarAnggota: [], daftarNoAnggota: [] },
          message: "Jenis tabungan belum memiliki akun.",
        });
      }
      ...
      const filteredJenis = jenis_tabungan_id
        ? jenisList.filter((j) => Number(j.id) === Number(jenis_tabungan_id))
        : jenisList;
      ...
      const totalTabungan = allData.reduce((sum, item) => sum + (parseFloat(item.total_tabungan) || 0), 0);
      ...
      return res.json({
        ...
        jenisTabungan: jenisList.map((jenis) => ({
          id: jenis.id,
          kode: jenis.kode,
          nama: jenis.nama,
          kolom_key: jenis.kolom_key,
      ...
      console.error("ERROR TABUNGAN:", error);
      return res.status(500).json({ message: "Gagal mengambil data tabungan", error: error.message });
    }
  }
  ...
  async export(req, res) {
    try {
      ...
      const jenisList = await this.getJenisTabungan();
      if (jenisList.length === 0) {
        return res.status(422).json({ message: "Belum ada jenis tabungan yang dikonfigurasi." });
      }
      ...
      if (!result) {
        return res.status(422).json({ message: "Jenis tabungan belum memiliki akun." });
      }
      ...
      const totalAkhir = data.reduce((sum, item) => sum + (parseFloat(item.total_tabungan) || 0), 0);

      if (exportType === "excel") {
        return this.exportExcel(res, data, kolomKeys, labels, totals, totalAkhir, "Tabungan");
      }
      if (exportType === "pdf") {
        return this.exportPdf(res, data, kolomKeys, labels, totals, totalAkhir, "Tabungan");
      }
      return res.status(400).json({ message: "Format export tidak didukung." });
    } catch (error) {
      console.error("ERROR EXPORT TABUNGAN:", error);
      return res.status(500).json({ message: "Gagal mengekspor data tabungan", error: error.message });
    }
  }
  ...
  async exportExcel(res, data, kolomKeys, labels, totals, totalAkhir, title = "Tabungan") {
    ...
    data.forEach((item) => {
      ...
      row.push(parseFloat(item.total_tabungan) || 0);
      sheet.addRow(row);
    });
  ...
  async exportPdf(res, data, kolomKeys, labels, totals, totalAkhir, title = "Tabungan") {
    ...
    data.forEach((item) => {
      ...
      row.push(formatRupiah(item.total_tabungan || 0));
      drawRow(row);
    });
