// src/pages/bendahara/RekapSimpananAnggotaPage.jsx
...
// ─── Komponen Utama ──────────────────────────────────────
export default function RekapSimpananAnggotaPage() {
  ...
  const [jenisSimpananId, setJenisSimpananId] = useState("");
  ...
  const fetchData = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        if (jenisSimpananId) params.jenis_simpanan_id = jenisSimpananId;

        const response = await api.get("/bendahara/rekap-simpanan-anggota", {
          params,
        });
    ...
    [selectedAnggota, tanggalMulai, tanggalSelesai, search, jenisSimpananId]
  );
  ...
  const loadAnggota = useCallback(async (searchQuery = "") => {
    ...
    try {
      ...
      setData((prev) => ({
        ...
        jenisSimpanan: response.data.jenisSimpanan || [],
      }));
    } catch (err) {
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      if (jenisSimpananId) params.append("jenis_simpanan_id", jenisSimpananId);
      params.append("export", type);

      const response = await api.get(
        `/bendahara/rekap-simpanan-anggota/export?${params.toString()}`,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.download = `rekap-simpanan-${selectedAnggota}-${new Date()
        .toISOString()
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-500">Riwayat simpanan per anggota</p>
            </div>
            {selectedAnggota && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-6 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Jenis Simpanan</label>
              <select
                value={jenisSimpananId}
                onChange={(e) => setJenisSimpananId(e.target.value)}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
              >
              ...
              <h3 className="font-semibold text-gray-800">
                ...
                {jenisSimpananId && (
                  <span className="ml-2 text-xs font-normal text-gray-500 bg-gray-200 px-2 py-0.5 rounded">
                    {jenisSimpanan.find((j) => j.id === parseInt(jenisSimpananId))?.nama || "Filtered"}
                  </span>
                )}
              </h3>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-gray-600">
                  <tr>
                    ...
                    <th className="py-2 px-4 text-left text-xs font-semibold uppercase tracking-wider">Jenis</th>
        ...
        {selectedAnggota && transaksi.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 text-center text-xs text-gray-400">
            Data rekap simpanan {selectedAnggota} • Periode:
