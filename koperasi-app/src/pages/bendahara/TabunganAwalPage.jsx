// src/pages/bendahara/TabunganAwalPage.jsx
...
export default function TabunganAwalPage() {
  // ─── State ──────────────────────────────────────────────────
  ...
  const getJenisLabel = (id) => {
    const found = jenisTabunganList.find((j) => j.id === id);
  ...
  const buildPivot = (rawData, jenisList) => {
    ...
    rawData.forEach((item) => {
      ...
      const jenisId = item.jenis_tabungan_id;
  ...
  const fetchData = useCallback(async (page = 1, activeFilters) => {
    ...
    try {
      ...
      const { data } = await api.get("/tabungan-awal", { params });

      setData(data.data);
      setPagination(data.pagination);
      setJenisTabunganList(data.jenisTabungan || []);

      const pivot = buildPivot(data.data, data.jenisTabungan || []);
      setPivotData(pivot);
  ...
  const openDetailModal = async (anggotaId, noAnggota, nama) => {
    ...
    try {
      const { data } = await api.get(`/tabungan-awal/anggota/${anggotaId}`);
      setDetailData(data.data || []);
    } catch (err) {
      alert("Gagal mengambil detail saldo awal tabungan anggota.");
    } finally {
  ...
  const handleDeleteFromDetail = async (item) => {
    ...
    const namaJenis = item.jenis_tabungan?.nama || item.jenis_tabungan || "Jenis";
    if (!window.confirm(`Yakin hapus saldo awal untuk ${namaAnggota} - ${namaJenis}?`)) return;
    try {
      await api.delete(`/tabungan-awal/${item.id}`);
      const { data } = await api.get(`/tabungan-awal/anggota/${detailAnggota.id}`);
      setDetailData(data.data || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await api.put(`/tabungan-awal/${editingId}`, payload);
      } else {
        payload = {
        ...
        await api.post("/tabungan-awal", payload);
      }
  ...
  const handleImportSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post("/tabungan-awal/import", formData, {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Saldo Awal Tabungan</h2>
              <p className="text-sm text-gray-500">Tabel pivot per anggota dan jenis tabungan</p>
            </div>
            <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-lg p-4 border">
                  ...
                  <p className="mt-2 text-xs text-gray-400 flex items-center gap-1">
                    <AlertCircle size={12} /> Format: no_anggota, jenis_tabungan (kode), tanggal, jumlah
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="overflow-x-auto rounded-xl bg-white shadow-sm border border-gray-100">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? (
                <tr>
                  <td colSpan={jenisTabunganList.length + 5} className="px-4 py-6 text-center text-gray-400">
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-white">
          <div className="flex items-center justify-between border-b px-4 py-4 sm:px-8">
            <h3 className="text-lg font-semibold text-gray-800 sm:text-xl">
              Saldo Awal Tabungan: {detailAnggota?.no_anggota} - {detailAnggota?.nama}
            </h3>
            <button
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50 text-left text-gray-600">
                    <tr>
                      <th className="px-4 py-3 text-center">NO</th>
                      <th className="px-4 py-3">Jenis Tabungan</th>
                  ...
                  <tbody className="divide-y">
                    {detailData.map((item, idx) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-3">{item.jenis_tabungan?.nama || "-"}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex flex-col bg-white">
          <div className="flex items-center justify-between border-b px-4 py-4 sm:px-8">
            <h3 className="text-lg font-semibold text-gray-800 sm:text-xl">
              {isEditing ? "Edit Saldo Awal Tabungan" : "Tambah Saldo Awal Tabungan"}
            </h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-8">
            <form id="tabungan-awal-form" onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-4">
              {error && (
              ...
              <div>
                ...
                {isEditing && (
                  <p className="mt-1 text-xs text-gray-400">
                    Jenis tabungan tidak dapat diubah setelah dibuat.
                  </p>
                )}
              </div>
              <div>
      ...
      {importModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6">
            <h3 className="text-lg font-semibold text-gray-800">Import Saldo Awal Tabungan</h3>
            <p className="text-sm text-gray-500 mt-1">
              Upload file Excel atau CSV dengan kolom: <strong>no_anggota, jenis_tabungan, tanggal, jumlah</strong>.
              Kolom <strong>jenis_tabungan</strong> diisi dengan kode jenis tabungan (contoh: TB), bukan nama.
            </p>
