// src/pages/bendahara/SimpananPage.jsx
...
export default function SimpananPage() {
  ...
  const [summary, setSummary] = useState({ saldo: {}, totalSimpanan: 0, jumlahAnggota: 0 });
  ...
  const fetchData = useCallback(async (page = 1, params = {}) => {
    ...
    try {
      ...
      const { data } = await api.get("/bendahara/simpanan", { params: queryParams });
      setData(data.data || []);
      setPagination(data.pagination || { page: 1, total_pages: 1, total: 0 });
      setSummary(data.summary || { saldo: {}, totalSimpanan: 0, jumlahAnggota: 0 });
      setJenisSimpanan(data.jenisSimpanan || []);
      ...
      if (data.filterActive) {
        ...
        setAppliedJenis(data.filterActive.jenis_simpanan_id || "");
        ...
        setFilterJenis(data.filterActive.jenis_simpanan_id || "");
        // Sync autocomplete
      ...
      console.error("Gagal fetch simpanan:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const applyFilters = () => {
    ...
    if (filterJenis) params.jenis_simpanan_id = filterJenis;
  ...
  const goToPage = (page) => {
    ...
    if (appliedJenis) params.jenis_simpanan_id = appliedJenis;
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      const response = await api.get("/bendahara/simpanan/export", {
        params,
        responseType: "blob",
      });
      ...
      link.href = url;
      link.download = `rekap-simpanan-${new Date().toISOString().slice(0, 10)}.${type === "excel" ? "xlsx" : "pdf"}`;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Rekap Simpanan Anggota</h2>
              <p className="text-sm text-gray-500">Saldo simpanan per anggota</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {jenisSimpanan.slice(0, 2).map((j, idx) => {
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">Anggota Menyimpan</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-800">Rp {formatRupiah(summary.totalSimpanan)}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 bg-white rounded-lg p-4 border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">Jenis Simpanan</label>
                      <select
                        value={filterJenis}
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={applyFilters} className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">
                      <Search size={15} /> Terapkan
                    </button>
                    <button onClick={resetFilters} className="flex items-center gap-1.5 px-4 py-2 border rounded-lg text-sm hover:bg-gray-50">
                ...
                <div className="bg-white rounded-lg p-4 border">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={() => handleExport("excel")} disabled={exporting} className="flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 disabled:opacity-60">
                      <FileSpreadsheet size={15} /> {exporting ? "Mengekspor..." : "Excel"}
                    </button>
                    <button onClick={() => handleExport("pdf")} disabled={exporting} className="flex items-center justify-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 disabled:opacity-60">
                      <FileText size={15} /> {exporting ? "Mengekspor..." : "PDF"}
                    </button>
                  </div>
                  <p className="mt-2 text-xs text-gray-400 flex items-center gap-1">
                    <AlertCircle size={12} /> Export dengan filter aktif
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex gap-2">
              <button onClick={applyFilters} className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                <Search size={15} /> Cari
              </button>
              <button onClick={resetFilters} className="flex items-center gap-1.5 rounded-lg border px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
                <XCircle size={15} /> Reset
              </button>
            </div>
          </div>
          {(appliedNama || appliedNoAnggota || appliedDari || appliedSampai || appliedSearch || appliedJenis) && (
            <p className="mt-3 text-sm text-gray-500">
              ...
              {appliedJenis && <span className="font-medium">Jenis: {jenisSimpanan.find(j => String(j.id) === appliedJenis)?.nama || appliedJenis}</span>}
            </p>
          )}
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-left text-gray-600">
              <tr>
                ...
                {jenisSimpanan.map((j) => (
                  <th key={j.kolom_key} className="px-4 py-3 text-right">{j.nama}</th>
                ))}
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={jenisSimpanan.length + 4} className="px-4 py-6 text-center text-gray-400">
                ...
                data.map((item, idx) => (
                  <tr key={item.id || idx} className="hover:bg-gray-50 transition">
                    ...
                    {jenisSimpanan.map((j) => (
                      <td key={j.kolom_key} className="px-4 py-3 text-right font-mono">
                        {formatRupiah(item[j.kolom_key] || 0)}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-right font-bold text-green-700">
                      Rp {formatRupiah(item.total_simpanan || 0)}
                    </td>
                  </tr>
                ))
              )}
                  ...
                  <td className="px-4 py-3 text-right font-mono text-green-700">
                    Rp {formatRupiah(
                      data.reduce((sum, item) => {
                        let totalItem = 0;
                        jenisSimpanan.forEach(j => {
                          totalItem += parseFloat(item[j.kolom_key]) || 0;
                        });
              ...
              <div key={item.id || Math.random()} className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{item.no_anggota}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-500">Total</p>
                    <p className="font-bold text-green-700">Rp {formatRupiah(item.total_simpanan || 0)}</p>
                  </div>
                </div>
