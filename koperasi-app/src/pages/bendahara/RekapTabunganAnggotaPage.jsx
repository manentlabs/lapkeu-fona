// src/pages/bendahara/RekapTabunganAnggotaPage.jsx
...
// ─── Komponen Utama ──────────────────────────────────────
export default function RekapTabunganAnggotaPage() {
  ...
  const [data, setData] = useState({
    ...
    grandTotal: {},
    saldoAwal: 0,
    jenisTabungan: [], // <-- berubah dari jenisSimpanan
    namaAnggota: [],
    currentPage: 1,
    totalPages: 1,
    totalTransaksi: 0,
  ...
  const [jenisTabunganId, setJenisTabunganId] = useState(""); // <-- berubah
  ...
  const fetchData = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        if (jenisTabunganId) params.jenis_tabungan_id = jenisTabunganId; // <-- berubah

        const response = await api.get("/bendahara/rekap-tabungan-anggota", {
          params,
        });
    ...
    [selectedAnggota, tanggalMulai, tanggalSelesai, search, jenisTabunganId]
  );
  ...
  const loadAnggota = useCallback(async (searchQuery = "") => {
    ...
    try {
      ...
      setData((prev) => ({
        ...
        jenisTabungan: response.data.jenisTabungan || [], // <-- berubah
      }));
    } catch (err) {
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      if (jenisTabunganId) params.append("jenis_tabungan_id", jenisTabunganId);
      params.append("export", type);

      const response = await api.get(
        `/bendahara/rekap-tabungan-anggota/export?${params.toString()}`,
        { responseType: "blob" }
      );
      ...
      link.href = url;
      link.download = `rekap-tabungan-${selectedAnggota}-${new Date()
        .toISOString()
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-500">Riwayat tabungan per anggota</p>
            </div>
            {selectedAnggota && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-6 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Jenis Tabungan</label>
              <select
                value={jenisTabunganId}
                onChange={(e) => setJenisTabunganId(e.target.value)}
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
              >
              ...
              <h3 className="font-semibold text-gray-800">
                ...
                {jenisTabunganId && (
                  <span className="ml-2 text-xs font-normal text-gray-500 bg-gray-200 px-2 py-0.5 rounded">
                    {jenisTabungan.find((j) => j.id === parseInt(jenisTabunganId))?.nama || "Filtered"}
                  </span>
                )}
              </h3>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-gray-600">
                  <tr>
                    ...
                    <th className="py-2 px-4 text-left text-xs font-semibold uppercase tracking-wider">Jenis Tabungan</th>
                    <th className="py-2 px-4 text-left text-xs font-semibold uppercase tracking-wider">Uraian</th>
                    <th className="py-2 px-4 text-right text-xs font-semibold uppercase tracking-wider">Tambah</th>
                    <th className="py-2 px-4 text-right text-xs font-semibold uppercase tracking-wider">Kurang</th>
        ...
        {selectedAnggota && transaksi.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 text-center text-xs text-gray-400">
            Data rekap tabungan {selectedAnggota} • Periode:
