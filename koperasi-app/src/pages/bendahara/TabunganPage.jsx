// src/pages/bendahara/TabunganPage.jsx
...
export default function TabunganPage() {
  ...
  const [summary, setSummary] = useState({ saldo: {}, totalTabungan: 0, jumlahAnggota: 0 });
  ...
  const fetchData = useCallback(async (page = 1, params = {}) => {
    ...
    try {
      ...
      const { data } = await api.get("/bendahara/tabungan", { params: queryParams });
      setData(data.data || []);
      setPagination(data.pagination || { page: 1, total_pages: 1, total: 0 });
      setSummary(data.summary || { saldo: {}, totalTabungan: 0, jumlahAnggota: 0 });
      setJenisTabungan(data.jenisTabungan || []);
      ...
      if (data.filterActive) {
        ...
        setAppliedJenis(data.filterActive.jenis_tabungan_id || "");
        ...
        setFilterJenis(data.filterActive.jenis_tabungan_id || "");
  ...
  const applyFilters = () => {
    ...
    if (filterJenis) params.jenis_tabungan_id = filterJenis;
  ...
  const goToPage = (page) => {
    ...
    if (appliedJenis) params.jenis_tabungan_id = appliedJenis;
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      const response = await api.get("/bendahara/tabungan/export", {
        params,
        responseType: "blob",
      });
      ...
      link.href = url;
      link.download = `rekap-tabungan-${new Date().toISOString().slice(0, 10)}.${type === "excel" ? "xlsx" : "pdf"}`;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Rekap Tabungan Anggota</h2>
              <p className="text-sm text-gray-500">Saldo tabungan per anggota</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {jenisTabungan.slice(0, 2).map((j, idx) => {
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">Anggota Menabung</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-800">Rp {formatRupiah(summary.totalTabungan)}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 bg-white rounded-lg p-4 border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">Jenis Tabungan</label>
                      <select
                        value={filterJenis}
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
          ...
          {(appliedNama || appliedNoAnggota || appliedDari || appliedSampai || appliedSearch || appliedJenis) && (
            <p className="mt-3 text-sm text-gray-500">
              ...
              {appliedJenis && <span className="font-medium">Jenis: {jenisTabungan.find(j => String(j.id) === appliedJenis)?.nama || appliedJenis}</span>}
            </p>
          )}
        </div>
        ...
        <div className="hidden overflow-x-auto rounded-xl bg-white shadow-sm border border-gray-100 lg:block">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={jenisTabungan.length + 4} className="px-4 py-6 text-center text-gray-400">
                ...
                data.map((item, idx) => (
                  <tr key={item.id || idx} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-green-700">
                      Rp {formatRupiah(item.total_tabungan || 0)}
                    </td>
                  </tr>
                ))
              )}
              ...
              <div key={item.id || Math.random()} className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-bold text-green-700">Rp {formatRupiah(item.total_tabungan || 0)}</p>
                  </div>
                </div>
