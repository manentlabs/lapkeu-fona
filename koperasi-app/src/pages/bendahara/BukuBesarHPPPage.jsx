// src/pages/bendahara/BukuBesarHPPPage.jsx
...
export default function BukuBesarHPPPage() {
  ...
  const fetchData = useCallback(async (mulai = "", selesai = "") => {
    ...
    try {
      ...
      const response = await api.get("/bendahara/buku-besar-hpp", { params });
  ...
  useEffect(() => {
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const mulai = startOfMonth.toISOString().slice(0, 10);
    const selesai = now.toISOString().slice(0, 10);
    fetchData(mulai, selesai);
  }, [fetchData]);
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      const response = await api.get(`/bendahara/buku-besar-hpp/export?${params.toString()}`, {
        responseType: "blob",
      });
      ...
      link.download = `buku-besar-hpp-${new Date().toISOString().slice(0, 10)}.${ext}`;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Buku Besar HPP</h2>
              <p className="text-sm text-gray-500">Riwayat transaksi Harga Pokok Penjualan</p>
            </div>
          </div>
        </div>
        ...
        {akun && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div className="bg-white rounded-xl shadow-sm p-3 border border-gray-100 text-center">
              ...
              <p className="text-sm font-bold text-gray-700 truncate">{akun.kode_akun}</p>
              <p className="text-xs text-gray-500 truncate">{akun.nama_akun}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-3 border border-gray-100 text-center">
                  ...
                  {data.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="py-8 text-center text-gray-400">
                        Tidak ada transaksi HPP untuk periode ini.
                      </td>
                    </tr>
                  ) : (
                    data.map((row, idx) => (
        ...
        {!loading && !error && !akun && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-600">Akun HPP Tidak Ditemukan</h3>
            <p className="text-sm text-gray-400 mt-1">Pastikan ada akun dengan nama mengandung "HPP"</p>
          </div>
        )}
      </div>
    </DashboardLayout>
