// src/pages/anggota/AnggotaPiutangPage.jsx
import { useState, useEffect } from 'react';
import DashboardLayout from '../../components/DashboardLayout';
import api from '../../api/axios';
import { Loader } from 'lucide-react';

export default function AnggotaPiutangPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/anggota-koperasi/piutang');
        setData(res.data);
      } catch (err) {
        setError(err.response?.data?.message || 'Gagal mengambil data piutang');
      } finally {
        setLoading(false);
      }
    };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="bg-white p-5 rounded-xl shadow-sm border">
          <h2 className="text-xl font-semibold text-gray-800">Riwayat Piutang</h2>
          <p className="text-sm text-gray-500">Daftar piutang yang Anda miliki</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.keys(totalPerJenis).map((key) => (
            <div key={key} className="bg-white p-4 rounded-xl shadow-sm border">
              ...
              <p className="text-xl font-bold text-red-600">{formatRupiah(totalPerJenis[key])}</p>
            </div>
          ))}
