// src/pages/anggota/AnggotaTabunganPage.jsx
import { useState, useEffect } from 'react';
import DashboardLayout from '../../components/DashboardLayout';
import api from '../../api/axios';
import { Loader } from 'lucide-react';

export default function AnggotaTabunganPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/anggota-koperasi/tabungan');
        setData(res.data);
      } catch (err) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="bg-white p-5 rounded-xl shadow-sm border">
          <h2 className="text-xl font-semibold text-gray-800">Riwayat Tabungan</h2>
          <p className="text-sm text-gray-500">Daftar tabungan (hari raya, pendidikan, qurban, dll)</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.keys(totalPerJenis).map((key) => (
            <div key={key} className="bg-white p-4 rounded-xl shadow-sm border">
              ...
              <p className="text-xl font-bold text-purple-600">{formatRupiah(totalPerJenis[key])}</p>
            </div>
          ))}
