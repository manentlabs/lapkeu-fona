...
export default function JenisPiutangPage() {
  ...
  const fetchData = useCallback(async (showInactive) => {
    setLoading(true);
    try {
      const { data } = await api.get("/pengaturan/jenis-piutang", {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing) {
      if (!form.kode.trim()) {
        setError("Kode jenis piutang wajib diisi.");
        return;
      }
      if (!form.akun_id) {
        setError("Akun piutang wajib dipilih.");
        return;
      }
    }
    if (!form.nama.trim()) {
      setError("Nama jenis piutang wajib diisi.");
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        ...
        await api.put(`/pengaturan/jenis-piutang/${editingId}`, payload);
      } else {
        payload = {
        ...
        await api.post("/pengaturan/jenis-piutang", payload);
      }
  ...
  const toggleActive = async (item) => {
    try {
      await api.put(`/pengaturan/jenis-piutang/${item.id}`, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/pengaturan/jenis-piutang/${deleteItem.id}`);
      setDeleteItem(null);
      fetchData(includeInactive);
    } catch (err) {
      alert(err.response?.data?.message || "Gagal menghapus jenis piutang.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Jenis Piutang</h2>
            <p className="text-sm text-gray-500">
              Kelola kategori piutang yang tersedia untuk anggota koperasi.
            </p>
          </div>
          <button
        ...
        <label className="flex w-fit items-center gap-2 text-sm text-gray-600">
          ...
          />
          Tampilkan jenis piutang nonaktif
        </label>

        {/* Daftar jenis piutang */}
        <div className="overflow-hidden rounded-xl bg-white shadow-sm">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 px-5 py-10 text-center">
              ...
              <p className="text-sm text-gray-400">Belum ada jenis piutang.</p>
            </div>
          ) : (
          ...
          <div className="flex min-h-full items-center justify-center px-4 py-16">
            <div className="w-full max-w-md">
              <h3 className="mb-6 text-xl font-semibold text-gray-800">
                {isEditing ? "Edit Jenis Piutang" : "Tambah Jenis Piutang"}
              </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    {isEditing
                      ...
                      : "Kode unik untuk identifikasi bisnis (contoh: PT, PK, PL)."}
                  </p>
                </div>

                <div>
                  <label className="mb-1 block text-sm text-gray-700">
                    Nama Jenis Piutang <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.nama}
                    onChange={(e) => setForm({ ...form, nama: e.target.value })}
                    placeholder="Contoh: Piutang Anggota"
                ...
                <div>
                  <label className="mb-1 flex items-center gap-1.5 text-sm text-gray-700">
                    Akun Piutang <span className="text-red-500">*</span>
                    {isEditing && <Lock size={12} className="text-gray-400" />}
                  </label>
          ...
          <div className="flex min-h-full items-center justify-center px-4 py-16">
            <div className="w-full max-w-sm text-center">
              <p className="mb-1 text-gray-700">
                Yakin ingin menghapus jenis piutang{" "}
