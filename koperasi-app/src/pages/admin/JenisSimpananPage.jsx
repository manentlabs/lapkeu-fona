...
export default function JenisSimpananPage() {
  ...
  const fetchData = useCallback(async (showInactive) => {
    setLoading(true);
    try {
      const { data } = await api.get("/pengaturan/jenis-simpanan", {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing) {
      if (!form.kode.trim()) {
        setError("Kode jenis simpanan wajib diisi.");
        return;
      }
      if (!form.akun_id) {
        setError("Akun simpanan wajib dipilih.");
        return;
      }
    }
    if (!form.nama.trim()) {
      setError("Nama jenis simpanan wajib diisi.");
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        ...
        await api.put(`/pengaturan/jenis-simpanan/${editingId}`, payload);
      } else {
        payload = {
        ...
        await api.post("/pengaturan/jenis-simpanan", payload);
      }
  ...
  const toggleActive = async (item) => {
    try {
      await api.put(`/pengaturan/jenis-simpanan/${item.id}`, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/pengaturan/jenis-simpanan/${deleteItem.id}`);
      setDeleteItem(null);
      fetchData(includeInactive);
    } catch (err) {
      alert(err.response?.data?.message || "Gagal menghapus jenis simpanan.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Jenis Simpanan</h2>
            <p className="text-sm text-gray-500">
              Kelola kategori simpanan yang tersedia untuk anggota koperasi.
            </p>
          </div>
          <button
        ...
        <label className="flex w-fit items-center gap-2 text-sm text-gray-600">
          ...
          />
          Tampilkan jenis simpanan nonaktif
        </label>
        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-sm">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 px-5 py-10 text-center">
              <PiggyBank size={28} className="text-gray-300" />
              <p className="text-sm text-gray-400">Belum ada jenis simpanan.</p>
            </div>
          ) : (
      ...
      {/* Modal Tambah/Edit - Full Layar (bg putih menutupi seluruh layar), konten di tengah */}
      {modalOpen && (
          ...
          <div className="flex min-h-full items-center justify-center px-4 py-16">
            <div className="w-full max-w-md">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    {isEditing
                      ...
                      : "Kode unik untuk identifikasi bisnis (contoh: SP, SW, SS)."}
                  </p>
                </div>

                <div>
                  <label className="mb-1 block text-sm text-gray-700">
                    Nama Jenis Simpanan <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.nama}
                ...
                <div>
                  <label className="mb-1 flex items-center gap-1.5 text-sm text-gray-700">
                    Akun Simpanan <span className="text-red-500">*</span>
                    {isEditing && <Lock size={12} className="text-gray-400" />}
                  </label>
      ...
      {/* Konfirmasi hapus - Full Layar (bg putih menutupi seluruh layar), konten di tengah */}
      {deleteItem && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-white">
          <button
          ...
          <div className="flex min-h-full items-center justify-center px-4 py-16">
            <div className="w-full max-w-sm text-center">
              <p className="mb-1 text-gray-700">
                Yakin ingin menghapus jenis simpanan{" "}
